# Compute the Precision/Recall (PR) metric.
...
def compute_pr(opts, nhood_size, row_batch_size, col_batch_size):
    ...
    real_features = metric_utils.compute_feature_stats_for_dataset(
        ...
        rel_lo = 0, rel_hi = 0, capture_all = True).get_all_torch().to(torch.float16).to(opts.device)

    gen_features = metric_utils.compute_feature_stats_for_generator(
        opts = opts, detector_url = detector_url, detector_kwargs = detector_kwargs,
        rel_lo = 0, rel_hi = 1, capture_all = True).get_all_torch().to(torch.float16).to(opts.device)
