...
def get_res2channels(channel_base, channel_max):
    return lambda res: int(min(channel_base // (2.0 ** (res - 1)), channel_max))

# Linear layer
# ----------------------------------------------------------------------------

# Get/create a weight variable for a convolution or fully-connected layer
...
def dense_layer(x, dim, gain = 1, act = "linear", use_wscale = True, lrmul = 1, 
        weight_var = "weight", bias_var = "bias", bias = True, name = None):
    if name is not None:
        weight_var = f"{weight_var}_{name}"
...
def apply_bias_act(x, act = "linear", lrmul = 1, bias_var = "bias", name = None):
    if name is not None:
        bias_var = f"{bias_var}_{name}"
...
# - num: number of elements in the x set (e.g. number of positions HW)
...
def minibatch_stddev_layer(x, group_size = 4, num_new_channels = 1, dims = 2):
    ...
    y = tf.reshape(x, [group_size, -1, num_new_channels, shape[1]//num_new_channels, shape[2]] + last_dims) # [GMncHW]
...
def get_sinusoidal_encoding(size, dim, num = 2):
    ...
    if num == 2:
        c = tf.expand_dims(tf.linspace(-1.0, 1.0, size), axis = -1)
...
def get_positional_encoding(max_res, dim, pos_type = "sinus", init = "uniform", dir_num = 2, 
        shared = False, crop_ratio = None):
    embs = []
    ...
    for res in range(max_res + 1):
        with tf.variable_scope("pos_emb%d" % res):
            size = 2 ** res
            if pos_type == "sinus":
                emb = get_sinusoidal_encoding(size, dim, num = dir_num)
            elif pos_type == "linear":
                emb = get_linear_encoding(size, dim, num = dir_num)
            ...
            embs.append(misc.crop_tensor(emb, crop_ratio))
    return embs
...
def nnlayer(x, dim, act, lrmul = 1, y = None, ff = True, name = "", att_mask = None, **kwargs):
    shape = get_shape(x)
    _x = x

    if y is not None:
        x = transformer_layer(from_tensor = x, to_tensor = y, dim = dim, name = name, 
            att_mask = tf.expand_dims(att_mask, axis = 1), **kwargs)[0]
    ...
    if ff:
        ...
        with tf.variable_scope("Dense%s_0" % name):
            x = dense_layer(x, dim, act = act, lrmul = lrmul)
        with tf.variable_scope("Dense%s_1" % name):
            x = dense_layer(x, dim, lrmul = lrmul)
...
def mlp(x, resnet, layers_num, dim, act, lrmul, batch = True, sa = False, norm_type = None, **kwargs):
    shape = get_shape(x)

    if len(get_shape(x)) > 2:
            x = to_2d(x, "last" if batch else "first") 
...
def process_input(t, t_pos, t_len, name):
    ...
    if len(shape) > 3:
        ...
        if t_len is None:
            misc.error(f"If {name}_tensor has two dimensions, must specify {name}_len.")
...
def gate_attention(att_probs, tensor, pos, batch_size, num_heads, from_len, to_len, name, gate_bias = 0):
    gate = dense_layer(tensor, num_heads, name = f"{name}_cont")
    if pos is not None:
        gate += dense_layer(pos, num_heads, name = f"{name}_pos")
...
def integrate(tensor, tensor_len, control, integration, norm):
    ...
    bias = gain = control = dense_layer(control, control_dim, name = "out")
    if integration == "both":
...
def compute_centroids(_queries, queries, to_from, to_len, from_len, batch_size, num_heads, 
    ...
    if to_from is not None:
        ...
        if get_shape(to_from)[-2] < to_len:
            ...
            to_from = upsample_2d(tf.reshape(to_from, [batch_size * num_heads, *hw_shape, from_len]), factor = 2, data_format = "NHWC")
        ...
        if get_shape(to_from)[-1] < from_len:
            ...
            to_from = upsample_2d(tf.reshape(to_from, [batch_size * num_heads, to_len, *hw_shape]), factor = 2, data_format = "NCHW")
            to_from = tf.reshape(to_from, [batch_size, num_heads, to_len, from_len])
    ...
    if to_from is None or parametric:
        if parametric:
            ...
            to_centroids = dense_layer(queries, dim * num_heads, name = "key2")
            to_centroids = transpose_for_scores(to_centroids, batch_size, num_heads, to_len, dim)
...
def transformer_layer(
        ...
        from_pos  = None,   to_pos  = None,    # The positional encoding of the form/to tensors (optional)
        from_gate = False,  to_gate = False,   # Add sigmoid gate on from/to, so that info may not be sent/received
                                               # when gate is low (i.e. the attention probabilities may not sum to 1)
        # Additional options
        ...
        hw_shape       = None,                 # The shape (height/width) of the from_tensor features
        att_vars       = None,                 # K-means variables carried over from layer to layer (only when --kmeans)
    ...
    with tf.variable_scope(f"AttLayer_{name}"):
        # Compute queries, keys and values
        queries = dense_layer(from_tensor, dim, name = "query") # [B*F, N*H]
        keys    = dense_layer(to_tensor, dim, name = "key")     # [B*T, N*H]
        values  = dense_layer(to_tensor, dim, name = "value")   # [B*T, N*H]
        _queries = queries

        # Add positional encoding to queries and keys
        if from_pos is not None:
            queries += dense_layer(from_pos, dim, name = "from_pos")
        if to_pos is not None:
            keys += dense_layer(to_pos, dim, name = "to_pos")

        if kmeans:
            from_elements, to_centroids = compute_centroids(_queries, queries, to_from,
                to_len, from_len, batch_size, num_heads, size_head, hw_shape, parametric = not iterative)
        ...
        for i in range(kmeans_iters):
            with tf.variable_scope(f"iter_{i}"):
                if kmeans:
                    if i > 0:
                        # Compute relative weights of different 'from' elements for each 'to' centroid
    ...
    if hw_shape is not None:
        att_probs = tf.transpose(tf.reshape(att_probs, [-1, *hw_shape, to_len]), [0, 3, 1, 2]) # [NCHW]
...
# - same: whether to merge all images equally along all HW positions
# Returns the merged images [batch_size, C, H, W] and latents [batch_size, k, layers_num, dim]
def merge_images(x, ws, k, type, same = False):
    ...
    if ws is not None:
        ...
        ws = tf.tile(tf.reduce_sum(ws * scores, axis = 1, keepdims = True), [1, k, 1, 1]) # [B, k, L, D]
...
def Generator(
    z,                                        # First input: Latent vectors (z) [batch_size, z_dim]
    c,                                        # Second input (optional): Conditioning labels [batch_size, c_dim]
    # General arguments
    is_training             = False,          # Network is under training? Enables and disables specific features
    is_validation           = False,          # Network is under validation? Chooses which value to use for truncation_psi
    return_ws               = False,          # Return ws in addition to the images?
    take_ws                 = False,          # Use input field as ws (instead of z, skip mapping network)
    ...
    w_avg_beta             = 0.995,          # Decay for tracking the moving average of W during training. None = disable
    # Mixing
    ...
    **kwargs):                                # Arguments for sub-networks (mapping and synthesis)
    ...
    # Validate arguments
    assert not is_training or not is_template_graph
    assert isinstance(components, EasyDict)
    z = tf.cast(z, tf.float32)
    c = tf.cast(c, tf.float32)

    # Set options for training
    # Turn off truncation cutoff for training/validation (such that it's on only for visualization)
    if is_training or (truncation_psi is not None and not tflib.is_tf_expression(truncation_psi) and truncation_psi == 1):
        truncation_psi = None
    if is_training:
        truncation_cutoff = None
    if is_validation:
        truncation_psi, truncation_cutoff = None, None
    # Turn off update of w latent mean when not training
    if not is_training or (w_avg_beta is not None and not tflib.is_tf_expression(w_avg_beta) and w_avg_beta == 1):
        w_avg_beta = None
    ...
    # Useful variables
    k = kwargs["components_num"] + int(kwargs.get("transformer", False)) # k attention region-based latents + 1 global latent (like StyleGAN)
    z_dim = kwargs["z_dim"]
    w_dim = kwargs["w_dim"]
    ...
    if "mapping" not in components:
        components.mapping = tflib.Network("G_mapping", func_name = globals()[mapping_func],
            num_broadcast = num_layers, **kwargs)

    # If input is used for ws (skipping mapping network),
    # then need latent per each synthesis network layer, see StyleGAN for details.
    if take_ws:
        z.set_shape([None, k, num_layers, w_dim])
    else:
        z.set_shape([None, k, z_dim])
    batch_size = get_shape(z)[0]

    # Initialize trainable positional embeddings for k latent components
    ltnt_pos = get_embeddings(k - 1, w_dim, name = "ltnt_emb")
    # Initialize component dropout mask (disabled by default)
    component_mask = random_dp_binary([batch_size, k - 1], component_dropout)
    component_mask = tf.expand_dims(component_mask, axis = 1)

    # Setup variables
    w_avg = tf.get_variable("dlatent_avg", shape = [w_dim], 
    ...
    if take_ws:
        ...
        ws = components.mapping.get_output_for(z, c, ltnt_pos, 
            component_mask, is_training = is_training, **kwargs)
        ws = tf.cast(ws, tf.float32)

        # Update moving average of W latent space
        if w_avg_beta is not None:
            with tf.variable_scope("DlatentAvg"):
                batch_avg = tf.reduce_mean(ws[:, :, 0], axis = [0, 1])
                update_op = tf.assign(w_avg, tflib.lerp(batch_avg, w_avg, w_avg_beta))
        ...
        def mixing(z, ws, prob, num, idx):
            if prob is None or prob == 0:
                return ws
            with tf.variable_scope("StyleMix"):
                z2 = tf.random_normal(get_shape(z))
                ws2 = components.mapping.get_output_for(z2, c, ltnt_pos, 
                    component_mask, is_training = is_training, **kwargs)
                ws2 = tf.cast(ws2, tf.float32)
                mixing_cutoff = tf.cond(
                    tf.random_uniform([], 0.0, 1.0) < prob,
                    lambda: tf.random_uniform([], 1, num, dtype = tf.int32),
                    lambda: num)
                ws = tf.where(tf.broadcast_to(idx < mixing_cutoff, get_shape(ws)), ws, ws2)
            return ws

        # Perform style mixing regularization
        layer_idx = np.arange(num_layers)[np.newaxis, np.newaxis, :, np.newaxis]
        ws = mixing(z, ws, style_mixing, num_layers, layer_idx)
        # Perform component mixing regularization
        ltnt_idx = np.arange(k)[np.newaxis, :, np.newaxis, np.newaxis]
        ws = mixing(z, ws, component_mixing, k, ltnt_idx)
        ...
        if truncation_psi is not None:
            with tf.variable_scope("Truncation"):
                ...
                ws = tflib.lerp(w_avg, ws, layer_psi)

    # Evaluate synthesis network
    imgs_out, maps_out = components.synthesis.get_output_for(ws, ltnt_pos, component_mask,
    ...
    if return_ws:
        ws = tf.identity(ws, name = "dlatents_out")
        ret += (ws, ) # [batch_size, num_broadcast, k (components_num), w_dim]
...
def G_mapping(
    ...
    z,                                      # First input: Latent vectors (z) [batch_size, z_dim]
    c,                                      # Second input (optional): Conditioning labels [batch_size, c_dim]
    ltnt_pos,                               # Third input: Positional embeddings for latents
    ...
    z_dim                   = 512,          # Latent (z) dimensionality per component
    w_dim                   = 512,          # Disentangled latent (w) dimensionality
    c_dim                   = 0,            # Label dimensionality, 0 if no labels
    # General settings
    num_broadcast           = None,         # Output disentangled latent (w) as [batch_size, w_dim]
                                            # or [batch_size, num_broadcast, w_dim]
    normalize_latents       = True,         # Normalize latent vectors (z) before feeding them to the mapping layers?
    transformer             = False,        # Whereas the generator uses attention or not (i.e. GANformer or StyleGAN)
    # Mapping network
    mapping_num_layers      = 8,            # Number of mapping layers
    ...
    **_kwargs):                             # Ignore unrecognized keyword args
    ...
    k = components_num + int(global_latent) # k region-based latents + 1 global latent (like StyleGAN)
    ...
    # Inputs
    z.set_shape([None, k, z_dim])
    c.set_shape([None, c_dim])
    ltnt_pos.set_shape([k - 1, w_dim])
    component_mask.set_shape([None, 1, k - 1])
    ...
    if net_dim is None:
        net_dim = w_dim
        output_dim = None # Since mapping dimension is already w_dim
    else:
        output_dim = w_dim
        # Map inputs to the mapping dimension
        x = to_2d(x, "last")
        x = dense_layer(x, net_dim, name = "map_start")
        x = tf.reshape(x, [batch_size, k, net_dim])
        if ltnt_pos is not None:
            ltnt_pos = dense_layer(ltnt_pos, net_dim, name = "map_pos")

    # Concatenate labels (False by default)
    if c_dim:
        with tf.variable_scope("LabelConcat"):
            w = tf.get_variable("weight", shape = [c_dim, z_dim], initializer = tf.initializers.random_normal())
            l = tf.tile(tf.expand_dims(tf.matmul(c, w), axis = 1), (1, k, 1))
            x = tf.concat([x, l], axis = 1)

    # Split latents to region-based and global
    if global_latent:
        x, g = tf.split(x, [k - 1, 1], axis = 1)
    ...
    if k - 1 == 0:
        ...
        if shared_dim > 0:
            ...
            x = dense_layer(x, shared_dim, name = "inmap")
            # Map the latents to the w space
            x = mlp(x, resnet, num_layers, shared_dim, act, lrmul, **mlp_kwargs)
            # Split the latents back to the k components
            x = tf.reshape(dense_layer(x, (k - 1) * net_dim, name = "outmap"), [batch_size, k, net_dim])
        else:
            x = mlp(x, resnet, num_layers, net_dim, act, lrmul,
                att_mask = component_mask, **mlp_kwargs)

    if global_latent:
        with tf.variable_scope("global"):
            # Map global latent to the w space
            g = mlp(g, resnet, num_layers, net_dim, act, lrmul)
    ...
    if output_dim is not None:
        x = tf.reshape(dense_layer(to_2d(x, "last"), output_dim, name = "map_end"),
            [batch_size, k, output_dim])

    # Broadcast latents to all layers (so to control each of the resolution layers of the Synthesis network)
    if num_broadcast is not None:
        with tf.variable_scope("Broadcast"):
            x = tf.tile(x[:, :, np.newaxis], [1, 1, num_broadcast, 1])

    # Output
    x = tf.identity(x, name = "dlatents_out")
    return x # [batch_size, num_broadcast, k (components_num), w_dim]
...
def G_synthesis(
    # Tensor inputs
    ws,                                 # First input: Disentangled latents (W) [batch_size, num_layers, w_dim]
    ltnt_pos,                           # Second input: Positional embeddings for latents
    ...
    channel_base        = 16 << 10,     # Overall multiplier for the network dimension
    channel_max         = 512,          # Maximum network dimension in any layer
    ...
    noise_mode          = "random",     # Options are random (randomize noise for every sample), const (use the same noise map) 
                                        # or none (no noise)
    ...
    **_kwargs):                         # Ignore unrecognized keyword args
    ...
    # Set variables
    k = components_num + int(transformer)
    act = nonlinearity
    ...
    # Network dimension as a function of the features resolution
    channels_num = get_res2channels(channel_base, channel_max)

    # Get global latent (the last one)
    def get_global(ws, res):
        if merge and res <= m:
            return tf.reshape(ws, [-1, num_layers, w_dim])
        return ws[:, -1]
    ...
    # Inputs
    ws.set_shape([None, k, num_layers, w_dim])
    component_mask.set_shape([None, 1, k - 1])
    ltnt_pos.set_shape([k - 1, w_dim])
    # Disable ltnt_pos if we don't want to use it
    if not use_pos:
        ltnt_pos = None

    batch_size = get_shape(ws)[0]

    # Positional encodings (spatial)
    # Image positional encodings, dictionary at different resolutions (used in GANformer)
    grid_poses = get_positional_encoding(resolution_log2, pos_dim, pos_type, pos_init, pos_directions_num, crop_ratio)
    ...
    for layer_idx in range(num_layers - 1):
        ...
        if merge and res < m:
            batch_multiplier = k
        noise_shape = [batch_multiplier, 1, *misc.crop_tensor_shape((2**res, 2**res), crop_ratio)]
    ...
    def layer(x, w, layer_idx, dim, kernel, att_vars, up = False):
        ...
        x = modulated_conv2d_layer(x, global_w, dim, kernel, up = up,
        ...
        if transformer and res >= start_res and res < end_res:
            # Reshape image features for transformer layer [batch_size (B), img_feats (HW), feat_dim (C)]
            ...
            kwargs = {
                ...
                "att_mask": tf.expand_dims(component_mask, axis = 1), # Attention mask (to disable particular latents)
                ...
                "to_pos": ltnt_pos,             # Positional encodings for the latents
                ...
                "hw_shape": (shape[2], shape[3]), # The shape (height/width) of the features
            ...
            x, att_map, att_vars = transformer_layer(from_tensor = x, to_tensor = w, dim = dim,
                name = "l2n", **kwargs)
            ...
            kwargs = {
                "num_heads": num_heads,   "att_dp": attention_dropout,
                "from_gate": ltnt_gate,   "from_pos": ltnt_pos
            }

            # Latent->Latent self-attention
            if ltnt2ltnt:
                new_w = nnlayer(w, w_dim, y = w, act = "lrelu",
                    name = "l2l", to_pos = ltnt_pos, to_gate = ltnt_gate, **kwargs)

            # Bottom-Up: Image->Latent attention
            if img2ltnt:
                new_w = nnlayer(w, w_dim, y = x, act = "lrelu",
        ...
        if local_noise and noise_mode != "none":
            x = add_noise(x, layer_idx)
        # Biases and nonlinearity
        x = apply_bias_act(x, act = act)

        return x, new_w, att_map, att_vars

    # The building block for the generator (two layers with upsampling and a resnet connection)
    def block(x, res, w, dim, att_vars, up = True): # res = 3..resolution_log2
        t = x
        with tf.variable_scope("Conv0_up"):
            x, w, att_map1, att_vars = layer(x, w, layer_idx = res*2-5,
                dim = dim, kernel = 3, up = up, att_vars = att_vars)

        with tf.variable_scope("Conv1"):
            x, w, att_map2, att_vars = layer(x, w, layer_idx = res*2-4,
    ...
    def torgb(t, y, res, global_ws): # res = 2..resolution_log2
            ...
            if res == resolution_log2:
                ...
                if transformer and res <= end_res:
                    with tf.variable_scope("extraLayer"):
                        t = modulated_conv2d_layer(t, global_ws[:, res*2-3], dim = channels_num(res),
                            kernel = 3, fused_modconv = fused_modconv, modulate = style)

            # Convert image features to output image (with num_channels, e.g. RGB)
            t = modulated_conv2d_layer(t, global_ws[:, res*2-2], dim = num_channels,
    ...
    with tf.variable_scope("4x4"):
        init_shape = misc.crop_tensor_shape((4, 4), crop_ratio)

        # Default option, trainable constant 4x4 image grid stem
        if not latent_stem:
            with tf.variable_scope("Const"):
                stem_size = k if merge else 1
                x = tf.get_variable("const", shape = [stem_size, channels_num(2), *init_shape],
        ...
        # If latent_stem, map the latents to an initial 4x4 image grid
            ...
            with tf.variable_scope("Dense"):
                x = dense_layer(get_global(ws, res)[:, 0], 
                    dim = channels_num(2) * np.prod(init_shape), 
                    bias = False, gain = np.sqrt(2) / 4)
                x = tf.reshape(x, [-1, channels_num(2), *init_shape])
                if local_noise and noise_mode != "none":
                    x = add_noise(x, 0)
                x = apply_bias_act(x, act = act)
        ...
        with tf.variable_scope("Conv"):
            x, w, att_map, att_vars = layer(x, w, layer_idx = 0, dim = channels_num(2),
                kernel = 3, att_vars = att_vars)
            att_maps.append(att_map)
        if architecture == "skip":
            imgs_out = torgb(x, imgs_out, 2, get_global(ws, res))
    ...
    for res in range(3, resolution_log2 + 1):
        with tf.variable_scope("%dx%d" % (2**res, 2**res)):
            ...
            x, w, _att_maps, att_vars = block(x, res, w, dim = channels_num(res), att_vars = att_vars)
            att_maps += _att_maps

            # Merge images, for k-GAN only, not used by default
            if merge and (not merged) and res >= m:
                with tf.variable_scope("merge%s" % (res)):
                    x, ws = merge_images(x, ws, k, merge_type, merge_same)
                merged = True
            ...
            if architecture == "skip" or res == resolution_log2:
                ...
                imgs_out = torgb(x, imgs_out, res, get_global(ws, res))
...
def Discriminator(
    ...
    c,                                  # Second input: Labels [batch_size, c_dim]
    # Dimensions and resolution
    a_dim               = 512,          # Aggregator variables dimension (only for using transformer in discriminator)
    c_dim               = 0,            # Dimensionality of the labels, 0 if no labels
    pos_dim             = None,         # Positional embeddings dimension
    ...
    **_kwargs):                         # Ignore unrecognized keyword args
    ...
    # Inputs
    images_in.set_shape([None, num_channels, *misc.crop_tensor_shape((resolution, resolution), crop_ratio)])
    c.set_shape([None, c_dim])
    ...
    ltnt_pos = None
    if use_pos:
        ltnt_pos = get_embeddings(components_num, a_dim, name = "ltnt_emb")
    ...
    def fromrgb(x, y, res): # res = 2..resolution_log2
        with tf.variable_scope("FromRGB"):
            t = apply_bias_act(conv2d_layer(y, dim = channels_num(res), kernel = 1), act = act)
            # Optional skip connections (see StyleGAN)
            if x is not None:
                t += x
            return t
    ...
    def block(x, res, aggregators): # res = 2..resolution_log2
        ...
        if transformer and res >= start_res and res < end_res:
            ...
            kwargs = {
                ...
                "from_pos": ltnt_pos,    "to_pos": grid_poses[res]
            }
            aggregators = transformer_layer(from_tensor = aggregators, to_tensor = x, dim = a_dim, name = "n2l", **kwargs)[0]

            kwargs.update({"to_gate": ltnt_gate, "to_pos": ltnt_pos})
            ...
            if ltnt2ltnt:
                kwargs.update({"from_gate": ltnt_gate, "from_pos": ltnt_pos})
                aggregators = nnlayer(aggregators, a_dim, act = "lrelu", y = aggregators, name = "l2l", **kwargs)

            # Image->Image self-attention (SAGAN)
            if img2img > 0 and res == img2img:
                kwargs["from_pos"]  = kwargs["to_pos"]  = grid_poses[res]
                kwargs["from_gate"] = kwargs["to_gate"] = img_gate
                x = transformer_layer(from_tensor = x, to_tensor = x, dim = a_dim, name = "n2n", **kwargs)[0]
        ...
        if local_att:
            x = att2d_layer(x, channels_num(res), 3, att2d_grid)

        ############################# Convolution layers (standard) ############################
        # Two convolution layers with downsmapling and a resnet connection
        with tf.variable_scope("Conv0"):
            x = apply_bias_act(conv2d_layer(x, dim = channels_num(res), kernel = ksize), act = act)

        with tf.variable_scope("Conv1_down"):
            x = apply_bias_act(conv2d_layer(x, dim = channels_num(res-1), kernel = ksize, down = True,
                resample_kernel = resample_kernel), act = act)

        if architecture == "resnet":
            with tf.variable_scope("Skip"):
                t = conv2d_layer(t, dim = channels_num(res-1), kernel = 1, down = True, resample_kernel = resample_kernel)
                x = (x + t) * (1 / np.sqrt(2))
    ...
    aggregators = None
    if transformer:
        aggregators = tf.get_variable(name = "aggregators", shape = [components_num, a_dim],
            initializer = tf.random_uniform_initializer())
    ...
    with tf.variable_scope("4x4"):
        ...
        if mbstd_group_size > 1:
            with tf.variable_scope("MinibatchStddev"):
                x = minibatch_stddev_layer(x, mbstd_group_size, mbstd_num_channels)
        with tf.variable_scope("Conv"):
            x = apply_bias_act(conv2d_layer(x, dim = channels_num(2), kernel = 3), act = act)
        # Turn final 4x4 image grid to a vector
        with tf.variable_scope("Dense0"):
            x = dense_layer(x, dim = channels_num(1), act = act) # [batch_size, channels_num(1)]
        ...
        with tf.variable_scope("ComponentScores"):
            if mbstd_group_size > 1:
                with tf.variable_scope("MinibatchStddev"):
                    ...
                    aggregators = minibatch_stddev_layer(aggregators, mbstd_group_size, mbstd_num_channels, dims = 1)
                    aggregators = tf.transpose(aggregators, [0, 2, 1])

            shape = get_shape(aggregators) # [batch_size, k, dim]

            with tf.variable_scope("Dense"):
                aggregators = dense_layer(to_2d(aggregators, "last"), a_dim, act = act) # [batch_size * k, dim]
            with tf.variable_scope("Output"):
                o = dense_layer(aggregators, 1) # [batch_size * k, 1]

            o = tf.reshape(o, shape[:-1]) # [batch_size, k]
            x = tf.concat([x, o], axis = -1) # [batch_size, channels_num(1) + k]

    # Output layer with label conditioning from "Which Training Methods for GANs do actually Converge?"
    with tf.variable_scope("Output"):
        x = dense_layer(x, dim = max(c.shape[1], 1))
        if c.shape[1] > 0:
            x = tf.reduce_sum(x * c, axis = 1, keepdims = True) # [batch_size, 1]
